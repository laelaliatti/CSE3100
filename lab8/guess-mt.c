...
void * thread_c(void * arg_in) //consumer
{
    ...
    pthread_mutex_unlock(&arg->mutex);  // Release mutex so thread_p can proceed
    /*do{
        pthread_mutex_lock(&arg->mutex);
        int guess;
        ...
        guess = arg->guess;
        arg->result = gmn_check(&gmn, guess);
    ...
    */
    while (1) {
        ...
        // Wait for guess from thread_p
        while (arg->status != S_GUESS) {
            pthread_cond_wait(&arg->cond_guess, &arg->mutex);
        }
        
        int guess = arg->guess;
        int check_result = gmn_check(&gmn, guess);
...
void * thread_p(void *arg_in) //producer
{
    thread_arg_t *arg = arg_in;
    ...
    do { 
        guess = (min + max)/2;
        printf("My guess: %d\n", guess);
        fflush(stdout); //forces the output to print immediately.
        // TODO
        ...
        while(arg->status != S_RESULT){
            ...
            result = arg->result;
            arg->status = S_INIT;
        pthread_mutex_unlock(&arg->mutex);  // Release mutex
...
int main(int argc, char *argv[])
{
    ...
    pthread_t th1;
    pthread_t th2;
    pthread_create(&th2, NULL, &thread_c, &arg);
    pthread_create(&th1, NULL, &thread_p, &arg);
