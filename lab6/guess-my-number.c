...
typedef  struct {
    int v;
    int num_attempts;
    char message[MSG_BUF_SIZE]; //Buffer is used for temporary storage.
} gmn_t;
...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int max = gmn_get_max();
    write(fdc[PIPEFD_WRITE], &max, sizeof(int));// this sends the max value to the parent
    ...
    while(correct !=0){
        ...
        correct = gmn_check(&gmn, guess);
        write(fdc[PIPEFD_WRITE], &correct, sizeof(int));
    }

    char *message = gmn_get_message(&gmn);
    write(fdc[PIPEFD_WRITE], message, MSG_BUF_SIZE);
