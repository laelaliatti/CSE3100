...
{
 //Have to do binary search because my O(n) method was too slow. 
 ...
 while (left <= right)
 {
    ...
    int compare = strcmp(word, words[mid]);
...
void decryption(unsigned char key, unsigned char shift, const int *encrypted, int len, char *decrypted)
{
    ...
    for(int i=0; i< len; i++){
        ...
        decrypted[i] = (char)(temp >> shift); //At the end (char) converts the result into a char because the shift goes to int.
    }

    decrypted[len] = '\0'; //indx len goes to the end and we add a NULL to tell the string is over
...
int message_score(const char *msg) // the higher the score the more accurate to the original message
{


    char temp[MAX];
    strcpy(temp, msg);  // this makes temp = msg
    int score = 0;
    char *token = strtok(temp, " \n\t\r,.:;!?"); // the symbols on the left are deliminers for the program to seperate the words

    while (token != NULL)
    {
        if(in_dict(token))
            score++; // if the word is in dictionary then append to score.

        token = strtok(NULL, " \t\n\r.,!?;:"); // when we call NULL it means to continue splitting prev string.
    }
...
int read_encrypted(char *filename, int *encrypted)
{
    ...
    if(fd < 0) //if there's nothing there
    {
        printf("Cannot open file %s\n", filename);
        exit(-1); //exits the program
    }

    int bytes = read(fd, encrypted, MAX *sizeof(int)); // reads the number of bytes in fd
    close(fd); //after you finish a file you need to close it
