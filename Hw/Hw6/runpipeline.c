...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    if (pid == 0){
        ...
        if (programs[cur].fd_in >=0) // if program reads.
            {
                if(dup2(programs[cur].fd_in, FD_STDIN)< 0){ //makes process read from wanted PID
                    die("dup2() failed.");
                }
                close(programs[cur].fd_in);
            }
         if (programs[cur].fd_out >=0) // if program writes.
            {
                if(dup2(programs[cur].fd_out, FD_STDOUT)< 0){ //makes process write to wanted PID
                    die("dup2() failed.");
                }
                close(programs[cur].fd_in);
            }
...
void prepare_pipes(Program *programs, int num_programs)
{
    ...
    for (int i = 0; i< num_programs - 1; i++){
        ...
        programs[i].fd_out = pipefds[PIPEFD_WRITE]; 
        programs[i+1].fd_in = pipefds[PIPEFD_READ]; 
    } 
}
