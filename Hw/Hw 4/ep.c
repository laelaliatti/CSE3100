...
enum TYPE {S, I, R};
// S susceptible hosts have not been infected
//I infected hosts have been infected and have not recovered
//R recovered hosts, !! they won't be infected again

//TODO: Implement idx
//idx returns an integer to be used for hashing
//this integer should be unique for every x, y pair in your grid
//k is the grid size parameter
//m is the number of hosts
//T is the duration parameter for an infected host to recover
//N is the size of the hash table.
int idx(int x, int y, int k)
{
	//x ∈ [-k, k]  x and y are in this range
	//y ∈ [-k, k]
	// so the total range is 2*k +1
	int size = 2 * k + 1;   //number of possible integers so like 2 gives us -2 -- 2 but we also need to include 0 so + 1
    int shifted_x = x + k;
    int shifted_y = y + k;
    return shifted_x * size + shifted_y;
}
...
{
	node *temp = *head; //we save the first node in a temp node
	*head = (*head)->next; //this essentially is moving the head forward
	temp->next = NULL; //This detaches temp
	return temp;
...
int location_match(node *head, THost host)
{
   ...
   while (head != NULL){
		...
		if(head_x == host.x && head_y == host.y)return 1;
...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
    ...
    // Step 1: Check for infections using current positions
    for(int i = 0; i < m; i++)
    {
        if(hosts[i].type == S)
    {
    ...
    // Step 3: Move all hosts with wraparound
    for(int i = 0; i < m; i++)
    {
        int r = rand() % 4;
        switch(r)
