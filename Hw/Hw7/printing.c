...
} job_queue_t; //think of like mutex
...
{
    // TODO
    printer_t *printer = (printer_t *)arg; //sets it as the struct
    job_queue_t *jobque = printer->jq; //-> cuz it's a pointer. 

    printer->njobs = 0;
    while(1){

        pthread_mutex_lock(&jobque->mutex);
        int r = q_num_jobs(jobque);
        if (r <= 0) {
            pthread_mutex_unlock(&jobque->mutex);
            break;
        }
        int job = q_fetch_job(jobque, printer->id);
        pthread_mutex_unlock(&jobque->mutex);
...
int main(int argc, char *argv[])
{
     ...
     *  */
    q_init(&job_queue, num_jobs);
    // initialize printers and create threads
