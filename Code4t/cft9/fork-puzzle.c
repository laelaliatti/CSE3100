...
int main(int argc, char *argv[])
{
    ...
    int n = 10; //at most 10 moves allowed

    for(int i = 0; i< n; i++)  //iterate through these 10 moves. 
    {
		pid_t pid; //pid is just what it's called think of it like a and then pid_t is data type
    	pid = fork();
        if(pid == 0) //child
        {   
            ...
            else if(cur + a[cur] >= 0 && cur + a[cur] <n)
            {
                ...
                if (moves >= 10){      //so we dont go out of bounds
                    return 0;
                } 
                else{
                    cur = cur + a[cur];   //moves right 
                    b[moves] = cur; //stores 
                    moves++; // keep moving
                }
