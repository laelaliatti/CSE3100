...
//Afterwards, each player saves their id and score into an array scores[], //and waits for everyone else to finish the game.
...
#include <assert.h>
#include <string.h>
//here the multiple readers are all the players reading from scores[].
...
void* player_func(void* threadarg)
{
	...
	for(int i=0; i< count; i++) //a whole lotta reading so we rdlock. 
	{
		if(scores[i].score > max_score)
		//because gradescope wasn't taking I had to add a tie breaking condition.
		//we make it so the one with the higherscore is the winner at the moment
		//if they're are two winners we pick the one with the greater id, as shown in the second part
		//(scores[i].score == max_score && scores[i].id > winner)
		{
...
int main(int argc, char *argv[])
{
    ...
    //pthread_rwlock_t rwlock;  giving my code problems. 
